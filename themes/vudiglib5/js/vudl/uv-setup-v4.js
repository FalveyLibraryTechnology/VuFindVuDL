...
function setupUV4(configUri, phpData, uvOptions) {
	...
	uv.on(UV.Events.LOAD, async function uvLoadEvent(e) {
		...
		if (renderings.length === 0) {
			for(const item of helper.manifest.__jsonld.items ?? []) {
				renderings = renderings.concat(getRenderings(item.rendering));
			}
		}
		...
		if (transcripts.length > 0) {
			...
			function addTranscriptsToRightPanel() {
				const formatItem = document.querySelector(
					".item._format, .item._language, .item._date-_added ",
				);
				if (formatItem) {
	...
	if (event.target.closest("#download-btn")) {
		waitFor(".overlay.download").then((overlay) => {
			if (overlay.querySelector(".attribution-text") === null) {
				const clonedAttribution = document.querySelector(".attribution-text").cloneNode(true);
				const attrTitleRegex = /(Digital Library@Villanova University)\s*(<br>)*\s*/;
				clonedAttribution.innerHTML = clonedAttribution.innerHTML.replace(attrTitleRegex, '<h2>Disclaimers/Usage</h2>');
				overlay
					.querySelector(".footer")
					.append(clonedAttribution);
			}
		});
	}
});
